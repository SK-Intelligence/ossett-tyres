...
const OETYRES_URL = "https://api.oneautoapi.com/driverightdata/oetyrefitmentdata/v2";

async function fetchDvla(reg) {
...
async function fetchOETyresRaw(vrm) {
  ...
  const res = await fetch(url, { method: "GET", headers: { "x-api-key": process.env.ONEAUTO_API_KEY } });
  let data = null;
  try {
    data = await res.json();
  } catch {
...
module.exports = { fetchDvla, fetchOETyresRaw };
